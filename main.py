...
def index_swipe():
    ...
    while 1:
        try:
            if poco(name="com.ss.android.ugc.aweme:id/iv_play").exists():
                print('出现上滑提示')
...
def page_search():
        ...
        data = dy_save_data.getone().decode()
...
def judge():
    if poco(text="推荐").exists():
        ...
        print('当前处于搜索页')
        page_search()
    elif poco(text="综合排序").exists() or poco(text="全部").exists():
        print('当前处于综合页')
        ...
        # poco.swipe([0.5, 0.8], [0.5, 0.2])
        # time.sleep(2)
    elif poco(text="个人信息保护指引").exists():
                print('出现个人信息保护指引')
                poco(text="同意").click()
                time.sleep(random.randint(2,3))
                if poco(text="拒绝").exists():
                    print('授权权限')
                    poco(text="拒绝").click()
                    # time.sleep(random.randint(1,2))
                    poco.swipe([0.5, 0.8], [0.5, 0.2],duration=0.5)
